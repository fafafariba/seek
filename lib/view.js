...
class View {
  ...
  splash(event) {
    $("#fariba").empty().addClass("hinge")
    .append('<a href="https://fafafariba.github.io"><div id="space">A </div><div>F</div><div>A</div><div>F</div><div>A</div><div>F</div><div>A</div><div>R</div><div>I</div><div>B</div><div id="space">A </div><div>PRODUCTION</div></a>');
    if (!this.intro) {
      this.intro = true;
  ...
  keyRouter(event){
    ...
    switch (event.key) {
      ...
      default:
        event.stopPropagation();
        console.log(this.game.letters.length, "length");
        console.log(this.game.letters, "letters");
        console.log(this.game.timer, "length");
